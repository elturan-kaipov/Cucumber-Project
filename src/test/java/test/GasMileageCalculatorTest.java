...
public class GasMileageCalculatorTest {
    ...
    public void dataDrivenMileageCalculatorTest() throws Exception {
        ...
        for (int rowNum = 1; rowNum < worksheet.getPhysicalNumberOfRows(); rowNum++) {
            ...
            double currentOdo=currentRow.getCell(0).getNumericCellValue();
...
//        gasObj.gasPrice.clear();
//        gasObj.gasPrice.sendKeys("3");

            gasObj.calculateButton.click();

            double actualdResult = (currentOdo - previousOdo) / gas;


            String[] expectedResult = gasObj.resultInGallon.getText().split(" ");
            System.out.println(expectedResult[0]);

            DecimalFormat decimalFormat = new DecimalFormat("#0.00");
            System.out.println(decimalFormat.format(actualdResult));

            if (currentRow.getCell(4)==null){
                currentRow.createCell(4);
            }

            currentRow.getCell(4).setCellValue(Boolean.parseBoolean(decimalFormat.format(actualdResult)));

            if(currentRow.getCell(5)==null){
                currentRow.createCell(5);
            }

            currentRow.getCell(5).setCellValue(Boolean.parseBoolean(expectedResult[0]));

            if (String.valueOf(decimalFormat.format(actualdResult)).equals(expectedResult[0])) {
                String pass="PASS!!!";
                if(currentRow.getCell(6)==null){
                    currentRow.createCell(6);
                }
                currentRow.getCell(6).setCellValue(Boolean.parseBoolean(pass));
            } else {
                String fail="FAIL!!!";
                if(currentRow.getCell(6)==null) {
                    currentRow.createCell(6);
                }
                currentRow.getCell(6).setCellValue(Boolean.parseBoolean(fail));
            }

            if(currentRow.getCell(7)==null){
                currentRow.createCell(7);
            }
            currentRow.getCell(7).setCellValue(Boolean.parseBoolean(LocalDateTime.now().toString()));
        }
